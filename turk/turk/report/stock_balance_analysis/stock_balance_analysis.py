...
def execute(filters=None):
	...
	agg_fields = ["opening_qty", "opening_val", "in_qty", "in_val", "purchase_qty", "purchase_val", "out_qty",
		"out_val", "sales_qty", "sales_val", "bal_qty", "bal_val", "val_rate"]
	...
	for (company, item, warehouse) in sorted(iwb_map):
		...
		for f in agg_fields:
			dataitems[key][f] += warehouse_item[f]
	...
	for (company, item) in sorted(iwb_map):
		if item_map.get(item):
			...
			if item_map[item]["pieces"] == pieces:
				...
				pieces = (availble_qty / (item_map[item]["boxes"] / item_map[item]["pieces"])) % item_map[item][
					"pieces"]
			...
			report_data = [item, item_map[item]["item_name"],
				...
				qty_dict.purchase_qty, qty_dict.purchase_val,
				qty_dict.out_qty, qty_dict.out_val,
				qty_dict.sales_qty, qty_dict.sales_val,
				qty_dict.bal_qty, reserved_stock[item]["reserved_qty"], availble_qty,
				boxes, pieces,
				qty_dict.bal_val, qty_dict.val_rate,
				item_map[item]["default_supplier"], item_map[item]["cust_supplier_name"], item_map[item]["item_color"],
				item_map[item]["size"], item_map[item]["texture"], item_map[item]["type"], item_map[item]["grade_tone_desc"],
				item_map[item]["cust_product_code"], item_map[item]["cust_company_code"],
				company
			]
...
def get_columns():
	...
	columns = [
		...
		_("In Qty") + ":Float:80",
		_("In Value")+":Float:80",
		_("Purchase Qty") + ":Float:80",
		_("Purchase Value")+":Float:80",
		_("Out Qty") + ":Float:80",
		_("Out Value")+":Float:80",
		_("Sales Qty") + ":Float:80",
		_("Sales Value")+":Float:80",
		...
		_("Supplier Name")+"::100",
		_("Color")+":Link/Item Color:100",
		_("Size")+":Link/Item Size:100",
		_("Texture")+":Link/Item Texture:100",
		_("Type")+":Link/Item Type:100",
		_("Grade/Tone")+"::100",
		_("Customer Product Code")+"::100",
		_("Customer Company Code")+"::100",
		...
		if d.posting_date < from_date:
			...
			if qty_diff > 0:
				...
				if d.voucher_type in ["Delivery Note", "Sales Invoice"]:
					qty_dict.sales_qty += abs(qty_diff)
					qty_dict.sales_val += abs(value_diff)
	...
	if items:
		for item in frappe.db.sql("""
			select name, item_name, description, item_group, brand, stock_uom , boxes, pieces,
				default_supplier, cust_supplier_name,
				item_color, size, texture, type, grade_tone_desc, cust_product_code, cust_company_code
			from `tabItem`

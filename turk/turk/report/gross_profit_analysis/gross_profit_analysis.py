...
def execute(filters=None):
	...
	def get_data(range1_data, range2_data):
		if filters.group_by == "Invoice":
			filters.group_by = "parent"

		# have the 2nd dataset with keys titled as [key2]
		grouped_data_2 = []

		for doc in range2_data:
			d = {}
			for key in doc.keys():
				d[key+"2"] = doc[key]
			grouped_data_2.append(d)

		# merge matched dataset-2 records with dataset-1 otherwise put blank values
		items_not_in_2 = []
		for range1_doc in range1_data:
			item_existed_in_2 = 0
			for range2_doc in grouped_data_2:
				if range1_doc[scrub(filters.group_by)] == range2_doc[scrub(filters.group_by)+"2"]:
					item_existed_in_2 = 1
					for key in range2_doc.keys():
						range1_doc[key] = range2_doc[key]
			if item_existed_in_2 == 0:
				items_not_in_2.append(range1_doc[scrub(filters.group_by)])
		for range1_doc in range1_data:
			if range1_doc[scrub(filters.group_by)] in items_not_in_2:
				range1_keys = [i for i in range1_doc.keys()]
				for key in range1_keys:
					range1_doc[key+"2"] = " "

		# check for unmatched records in dataset-2, if any put an empty doc for dataset-1 and place them at the bottom of the records
		for range2_doc in grouped_data_2:
			item_existed_in_1 = 0
			for range1_doc in range1_data:
				if range1_doc[scrub(filters.group_by)] == range2_doc[scrub(filters.group_by)+"2"]:
					item_existed_in_1 = 1
			if item_existed_in_1 == 0:
				doc1 = {}
				for i in range(2):
					for key in range2_doc.keys():
						k, val = key, range2_doc[key]
						if i == 0:
							k, val = key[:-1], " "
						doc1[k] = val
				range1_data.append(doc1)
		return range1_data


	range1_data = GrossProfitGenerator(filters).grouped_data
	filters.from_date = datetime.datetime.strptime(filters.comp_from_date, "%Y-%m-%d").date()
	filters.to_date = datetime.datetime.strptime(filters.comp_till_date, "%Y-%m-%d").date()
	range2_data = GrossProfitGenerator(filters).grouped_data


	if range1_data or range2_data:
		if range1_data and not range2_data:
			range2_data = []
			keys = [key for key in range1_data[0].keys()]
			for _ in range1_data:
				doc = {}
				for key in keys:
					doc[key+"2"] = " "
				range2_data.append(doc)
		elif range2_data and not range1_data:
			range1_data = []
			keys = [key for key in range2_data[0].keys()]
			for _ in range2_data:
				doc = {}
				for key in keys:
					doc[key] = " "
				range1_data.append(doc)

		grouped_data_1 = get_data(range1_data, range2_data)
	...
	group_wise_columns = frappe._dict({
		...
		"item_group": ["item_group", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount",
			"gross_profit", "gross_profit_percent"],
		"customer": ["customer", "customer_name", "customer_group", "qty", "base_rate", "buying_rate", "base_amount",
	...
	for src in grouped_data_1:
		lmq = flt(src["qty2"])
		lsp = flt(src["base_rate2"])
		base_rate = flt(src["base_rate"])
		# pv = abs((base_rate - lsp) * lmq)
		pv = abs((base_rate - lsp) * flt(src["qty"]))

		# if base_rate > 0:
		# 	qv = abs((flt(src["qty"]) - lmq)*base_rate)
		# else:
		# 	qv = abs(flt(src["qty"]) * base_rate)

		qv = (flt(src["qty"]) - lmq) * lsp
		tv = pv + qv
		ablm = abs(flt(src["buying_rate2"]))
		if ablm == 0:
			gbp = 0
		else:
			gbp = (flt(src["buying_rate"]) - ablm)/ablm
		if lsp == 0:
			gsp = 0
		else:
			gsp = (base_rate - lsp)/lsp
		diff = gsp - gbp
		row = []
		for i in range(2):
			for col in group_wise_columns.get(scrub(filters.group_by)):
				...
				if i==1:
					column_name = col+"2"
				row.append(src.get(column_name))
...
def get_columns(group_wise_columns, filters):
	...
	for i in range(2):
		for col in group_wise_columns.get(scrub(filters.group_by)):
			if ":" in column_map.get(col) and i == 1:
				col_title = column_map.get(col).split(":")
				col_title[0] = col_title[0]+'2'
				col_name = ':'.join(col_title)
			else:
				col_name = column_map.get(col)
			columns.append(col_name)

	columns.append({"fieldname": "currency", "label" : _("Currency"), "fieldtype": "Link", "options": "Currency"})

	lmq = {	"fieldname": "lmq", "label" : _("LMQ"), "fieldtype": "Float"}
	lsp = {	"fieldname": "lsp", "label" : _("LSP"), "fieldtype": "Float"}
	pv =  {	"fieldname": "pv", "label" : _("Price Variance"), "fieldtype": "Float"}
	qv =  {	"fieldname": "qv", "label" : _("Quantity Variance"), "fieldtype": "Float"}
	tv =  {	"fieldname": "tv", "label" : _("Total Variance"), "fieldtype": "Float"}
	ablm =  {"fieldname": "ablm", "label" : _("Avg Buying LM"), "fieldtype": "Float"}
	gbp =  {"fieldname": "gbp", "label" : _("Growth Buying %"), "fieldtype": "Float"}
	gsp =  {"fieldname": "gsp", "label" : _("Growth selling  %"), "fieldtype": "Float"}
	diff =  {"fieldname": "diff", "label" : _("Difference"), "fieldtype": "Float"}

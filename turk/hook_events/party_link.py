...
def create_invoice_adj_jv(self,cdt):
	if self.doctype == 'Sales Invoice' and self.is_pos == 0 and self.no_double_ledger==0:
		...
		doc = frappe.get_doc(self,cdt)
		supplier_list = frappe.get_list('Party Link', filters= {'primary_role':'Customer','secondary_party':self.supplier}, fields = "*" )
		customer_dp = None
		supplier_dp = None
		primary_role_dp = None
		if supplier_list:
			for msg in supplier_list:
				customer_dp = msg.primary_party
				supplier_dp = msg.secondary_party
				primary_role_dp = msg.primary_role
